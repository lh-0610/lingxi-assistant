...
src/                     # 主代码
  ...
  roles.py               # 角色卡加载 + get_system_prompt（拼角色卡 / 项目上下文 / .lingxirules / 记忆 / 按需画图规范）
  ...
  ui/                    # UI 包（chat_window.py 用 mixin 拆分，主文件从 3766 → ~1960 行）
    ...
    markdown_render.py   # MarkdownRenderMixin：_md_to_html / render_final_markdown(speak=) / 思考块管理
    ...
    settings_dialog.py   # 设置弹窗（provider 卡片式 API key + 自定义模型增删改 + GPT-SoVITS 启动）
    helpers.py           # _make_button_icon / _build_image_content_block / _strip_markdown_for_tts / _escape
...
# 语音依赖（可选）
pip install faster-whisper sounddevice soundfile av ctranslate2 huggingface-hub numpy
...
- **TTS**（`voice.py:GPTSoVITSTTS`）：
  - 之前用 QMediaPlayer 首次播放卡 25s（Qt FFmpeg 初始化），换成 sounddevice
  - 现在用**流式模式**：`streaming_mode=true&media_type=raw` → HTTP `iter_content(4096)` → `sd.OutputStream(samplerate=32000, channels=1, dtype=int16).write(chunk)`，第一句合成完就播
- **启动器**（`gpt_sovits_launcher.py:GPTSoVITSLauncher`）：
  - `subprocess.Popen(runtime/python.exe -u api_v2.py)` + `CREATE_NO_WINDOW`
  - 后台轮询 9880 端口（每秒一次，包装在 `Thread`），就绪后 HTTP POST `/set_gpt_weights` + `/set_sovits_weights`
  - **Windows Job Object + `KILL_ON_JOB_CLOSE`**：父进程 crash / 任务管理器强杀也能保证 api_v2.py 子进程不残留
- **TTS click 流程**（`src/ui/chat_window.py:_on_tts_click`）：开关 🔊 时若服务未启动 → `QMessageBox` 询问 → 校验 5 项必填配置 → `launcher.start()` → status_changed 信号 running 时自动开启 TTS 按钮

### 图片生成
- `generate_image` 工具优先调本机 ComfyUI（`comfy_base_url` 默认 9880... 笔误，是 `8188`）
- 工作流支持自定义 JSON（`comfy_workflow_path`），含 `PROMPT_PLACEHOLDER` 占位符自动替换
- 没 ComfyUI 时降级到 Pollinations.ai 在线 API
...
| `generate_video` | Agnes Video V2.0 文生/图生视频（`apihub.agnes-ai.com/v1/videos`，config `agnes_api_key`）：异步任务 POST 创建 → 轮询 → 下载 mp4 存项目 `outputs/`；没配 key 优雅降级；写类工具、Plan/遥控自动拦 |
...
- PyInstaller 打包配置见 `lingxi.spec`（产物 exe 名仍是中文「灵犀」）

### GPT-SoVITS 集成踩坑
- **Blackwell GPU（RTX 50 系，sm_120）**：整合包默认带的 PyTorch 没 sm_120 kernel，需手动升级到 cu128：
  ```
  ...
  ```
- 升级 PyTorch 后 `GPT_SoVITS/AR/modules/patched_mha_with_cache.py` 顶部需解开两行：
  ```python
  from torch import Tensor
  from typing import Callable, List, Optional, Tuple, Union
  ```
- API 用 `urllib.request.urlopen` 探活时 **404 必须当存活**（urlopen 默认在 4xx 抛 HTTPError）
- 中文路径在 cmd 下用 `curl` 会被错乱截断，**所以 set_weights 用 Python urllib 不要用 bat**
- emoji（如 `✨`）会让 GPT-SoVITS API 返 400（内部 GBK 编码），TTS 路径前必须 `.encode("gbk", errors="ignore").decode("gbk")` 过滤
